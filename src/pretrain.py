...
from model.lstm import LSTMModel, LSTM_Diffusion_Model
...
from src.util import parse_train_args, metric
...
if __name__ == "__main__":
    ...
    args.model = 'lstm_diffusion'  # 'mlp', 'lstm', 'dlinear', 'timesnet', 'ns_transformer'
    ...
    if args.is_training: # Training mode
        ...
        if args.model == 'ns_transformer':
            model = NsTransformer(args).to(device)
        ...
        elif args.model == 'lstm_diffusion':
            model = LSTM_Diffusion_Model(args).to(device)
        else:
